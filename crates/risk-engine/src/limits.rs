...
impl LimitTracker {
    ...
    /// WS1.7: Resets daily volume at UTC boundary before checking.
    pub fn check_trade(
        &mut self,
        trade: &TradeRequest,
        account: &Account,
        ...
        // WS1.7: Single maybe_reset_day called from both read and write paths
        ...
        if self.last_trade_epoch_secs > 0 {
            ...
            if elapsed < self.limits.cooldown_seconds {
                ...
                return Err(format!("cooldown_active: {}s remaining", remaining));
            }
        }
    ...
    /// WS1.7: Single reset method for both read and write paths.
    /// Atomically resets daily volume and updates day_start_epoch_secs.
...
mod tests {
    ...
    fn test_trade_within_limits() {
        let mut tracker = LimitTracker::new(default_limits());
    ...
    // ── WS1.7: UTC boundary fix tests ───────────────────────────────────

    #[test]
    fn test_check_trade_resets_volume_on_new_day() {
        let day1 = 86400 + 100;
        let mut tracker = LimitTracker::new_with_time(default_limits(), day1);
        let account = Account::new(Decimal::from_f64(10000.0));

        // Fill up volume on day 1
        tracker.record_trade(Decimal::from_f64(490.0), day1);
        assert_eq!(tracker.daily_volume.to_f64(), 490.0);

        // check_trade on day 2 should reset volume atomically
        let day2 = day1 + 86400;
        let trade = test_trade(0.01, 3000.0); // notional = $30
        let result = tracker.check_trade(&trade, &account, day2);
        assert!(result.is_ok());
        // Volume should have been reset
        assert_eq!(tracker.daily_volume.to_f64(), 0.0);
    }

    #[test]
    fn test_same_day_no_reset() {
        let now = 86400 + 100;
        let mut tracker = LimitTracker::new_with_time(default_limits(), now);
        let account = Account::new(Decimal::from_f64(10000.0));

        tracker.record_trade(Decimal::from_f64(50.0), now);
        assert_eq!(tracker.daily_volume.to_f64(), 50.0);

        let trade = test_trade(0.01, 3000.0);
        let _ = tracker.check_trade(&trade, &account, now + 100);
        // Volume should NOT be reset
        assert_eq!(tracker.daily_volume.to_f64(), 50.0);
    }
}
